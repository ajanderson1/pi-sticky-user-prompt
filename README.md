...
The tests cover fullscreen-settings admission, submitted-prompt rules, tail-following compensation,
manual scrolling, clamping at the document start, and repeated redraws at a prompt boundary.

The visual demo is fully reproducible. It boots an isolated Pi (its own `PI_CODING_AGENT_DIR` with
only this extension loaded), replays a pre-seeded session so no model calls happen on camera, drives
the TUI through tmux, and renders the GIF, MP4, and stills:
